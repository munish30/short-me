...
export default function Page({ params }: { params: { shortId: string } }) {
  ...
  const shortlink = `${process.env.NEXT_PUBLIC_URL}${params.shortId}`;
  ...
  return (
    <div className="w-[80%] h-[70%] bg-white rounded-sm p-5 flex flex-col gap-4 items-center justify-center">
      <div className="outline-1 flex flex-col justify-center items-center gap-2 p-5 w-full sm:w-[60%]">
        ...
        {/* Shortlink display and copy */}
        <div className="flex outline rounded-lg overflow-hidden w-full">
          <input readOnly value={shortlink} className="text-black p-2 pl-3 outline-none w-full" />
          <button
            onClick={handleCopy}
